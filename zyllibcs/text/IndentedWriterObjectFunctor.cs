...
namespace zyllibcs.text {
	...
	public class IndentedObjectFunctor {
		...
		public IndentedObjectFunctor(Type basetype, IndentedObjectFunctorOptions options, BindingFlags basebinding, IndentedWriterMemberOptions writeroptions, IEnumerable<IndentedWriterObjectProc> memberprocs, object userdata)
			: base() {
			m_BaseType = basetype;
			m_Options = options;
		...
		public bool WriterObject(IIndentedWriter iw, object obj) {
			return WriterObject_Core(iw, obj);
		}
		...
		/// <param name="obj">object. If <paramref name="obj"/> is null, result alway is false.</param>
		/// <returns>当<paramref name="iw"/>为null时, 返回是否支持输出. 否则返回是否成功输出.</returns>
		protected virtual bool WriterObject_Core(IIndentedWriter iw, object obj) {
			// check.
