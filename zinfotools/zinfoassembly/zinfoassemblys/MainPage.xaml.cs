...
//#endif
		...
		private void btnFind_Click(object sender, RoutedEventArgs e) {
			...
			// 设置焦点.
			try {
				txtInfo.Focus(Windows.UI.Xaml.FocusState.Keyboard);
			}
			catch (Exception ex) {
				Debug.WriteLine(ex);
			}
		...
		private async void btnSave_Click(object sender, RoutedEventArgs e) {
			FileSavePicker dlg = new FileSavePicker();
			dlg.FileTypeChoices.Add("Text file", new string[] { ".txt" });
			//dlg.FileTypeChoices.Add("All file", new string[] { ".*" });
			StorageFile file = await dlg.PickSaveFileAsync();
			if (null!=file) {
				await FileIO.WriteTextAsync(file, txtInfo.Text, Windows.Storage.Streams.UnicodeEncoding.Utf8);
			}
		}
